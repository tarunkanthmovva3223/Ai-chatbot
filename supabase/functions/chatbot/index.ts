...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const supabaseClient = createClient(Deno.get('SUPABASE_URL') ?? '', Deno.get('SUPABASE_SERVICE_ROLE_KEY') ?? '', {
      auth: {
    ...
    const { data: chat, error: chatError } = await supabaseClient.from('chats').select('id').eq('id', chat_id).eq('user_id', user_id).single();
    if (chatError || !chat) {
    ...
    const { data: botMessageData, error: insertError } = await supabaseClient.from('messages').insert([
      {
        chat_id,
        content: botMessage,
        is_bot: true,
